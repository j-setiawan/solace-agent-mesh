...
async def test_submit_prompt_and_get_response(
    ...
    """
    Tests a basic interaction: submitting a prompt to the webhook gateway
    ...
    task_id = await test_webhook_gateway_component.send_test_input(test_input)
    assert task_id is not None

    # 3. Assert: Retrieve the captured output and verify it
    all_events = await get_all_task_events(
        test_webhook_gateway_component, task_id, overall_timeout=10.0
    )
    terminal_event, _, terminal_event_text = extract_outputs_from_event_list(
        all_events, f"test_webhook_gateway_{task_id}"
    )
...
async def test_submit_prompt_and_get_streaming_response(
    ...
    terminal_event, intermediate_events, terminal_event_text = (
        extract_outputs_from_event_list(all_events, f"test_webhook_gateway_{task_id}")
    )
...
async def test_submit_concurrent_requests(
    ...
    # 2. Act
    tasks = [
        test_webhook_gateway_component.send_test_input(input_data)
...
async def test_submit_request_with_missing_target_agent(
    ...
    with pytest.raises(ValueError, match="Test input must specify 'target_agent_name'"):
        await test_webhook_gateway_component.send_test_input(test_input)
...
async def test_component_instantiation(
    ...
    """
    Tests that the TestWebhookGatewayComponent can be instantiated without errors.
    """
    assert test_webhook_gateway_component is not None
    assert isinstance(test_webhook_gateway_component, TestWebhookGatewayComponent)
