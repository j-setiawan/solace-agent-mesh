...
async def test_submit_prompt_and_get_response(
    ...
    """
    Tests a basic interaction: submitting a prompt to the web gateway
    ...
    task_id = await test_web_gateway_component.send_test_input(test_input)
...
async def test_submit_request_with_missing_target_agent(
    ...
    with pytest.raises(ValueError, match="Test input must specify 'target_agent_name'"):
        await test_web_gateway_component.send_test_input(test_input)
...
async def test_component_instantiation(
    ...
    """
    Tests that the TestWebGatewayComponent can be instantiated without errors.
    """
    assert test_web_gateway_component is not None
    assert isinstance(test_web_gateway_component, TestWebGatewayComponent)
